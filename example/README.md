...
4. Run `flutter pub get && (cd ios && pod install || pod update)` within this directory.
5. In `lib/main.dart` choose prefered login configuration(`loginType` variable) and provide your login data according to it: username and password for `credentials`, tokenUrl for `token`, apiKey for `apiKey`.
